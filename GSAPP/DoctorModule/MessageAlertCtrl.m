...
- (void)getModel
{
   // if ([[UserDataManager shareManager].userType isEqualToString:ExpertType]) {
        ...
        [[NetworkManager shareMgr] server_fetchConsultWithDic:dicPrivate completeHandle:^(NSDictionary *response) {
           ...
           NSDictionary *parameters = @{@"expand": @"consultation,doctor",@"where[order_doctor_id][]":[UserDataManager shareManager].userId, @"where[status]": @[@2, @3, @6]};
            ...
            [[NetworkManager shareMgr] server_fetchOrderWithDic:dicPrivate1 completeHandle:^(NSDictionary *response) {
                ...
                if (resultArray1.count != 0) {
                    ...
                    [self.arrayDoctorOrderStatusIsEqualToOne addObjectsFromArray:resultArray1];
                                ...
                                NSDictionary *parameters = @{@"expand": @"consultation,orderDoctor",@"where[doctor_id][]":[UserDataManager shareManager].userId, @"where[status]": @[@2, @4, @6]};
//                                NSDictionary* dicPrivate2 = [NSDictionary dictionaryWithObjectsAndKeys:[UserDataManager shareManager].userId,@"where[doctor_id][]",@"consultation,orderDoctor",@"expand",@"4",@"where[status]",nil];
                                [[NetworkManager shareMgr] server_fetchOrderWithDic:parameters completeHandle:^(NSDictionary *response1) {
                                    ...
                                    self.arrayDoctorOrderStatusIsEqualToTwo = [[NSMutableArray alloc] init];;
                                    ...
                                    if (resultArray1.count != 0) {
                                        ...
                                        for (int i = 0; i < resultArray1.count; i ++) {
                                            ...
                                            if (order.status == 2) {
                                                ...
                                                [self.arrayDoctorOrderStatusIsEqualToTwo addObject:[resultArray1 objectAtIndex:i]];
...
//        [cell.img_goNext setImage:[UIImage imageNamed:@"list_more"]];
        ...
        cell.txtIntro.text = [NSString stringWithFormat:@"您已经被%@选择了%@",order.doctor.name,order.consultation.patient_illness] ;
...
//        [cell.img_goNext setImage:[UIImage imageNamed:@"list_more_pre"]];
        ...
        cell.lblTime.text = order.consultation.created_at;
        cell.txtIntro.text = [NSString stringWithFormat:@"%@医生取消了案例%@",order.doctor.name,order.consultation.patient_illness];
    }else if (indexPath.section == 3) {
        
        GSOrder* order = [GSOrder objectWithKeyValues:[self.arrayExpertOrderStatusIsEqualThree objectAtIndex:indexPath.row]];
        
        cell.lbl_hintOfMessage.text = @"已付款提示";//[NSString stringWithFormat:@"%@医生已经为案例%@付款",order.doctor.name,order.consultation.patient_illness];
        
//        [cell.lbl_hintOfMessage setTextColor:[UIColor colorWithRed:79.0/255.0 green:193.0/255.0 blue:233.0/255.0 alpha:1.0]];
//        [cell.img_goNext setImage:[UIImage imageNamed:@"list_more_pre"]];
        
        cell.lblTime.text = order.consultation.created_at;
        cell.txtIntro.text = [NSString stringWithFormat:@"%@医生已经为案例%@付款",order.doctor.name,order.consultation.patient_illness];
...
//        [cell.img_goNext setImage:[UIImage imageNamed:@"list_more_pre"]];
        ...
        cell.lblTime.text = order.consultation.created_at;
             cell.txtIntro.text = [NSString stringWithFormat:@"您的%@案例尚未付款",order.consultation.patient_illness];
